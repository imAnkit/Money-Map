...
class EditBillsScreen extends StatefulWidget {
  final Bills bill;
  const EditBillsScreen({super.key, required this.bill});
...
class _EditBillsScreenState extends State<EditBillsScreen>
  ...
  void initState() {
    ...
    toRemind = widget.bill.remind;
    _selectedFrequency = widget.bill.frequency;
    days = widget.bill.daysBefore!;
    _selectedTime = widget.bill.time;

    _selectedFrequency = widget.bill.frequency;
    _dateController.text = DateFormat('dd/MM/yyyy').format(widget.bill.date);
    newBill = widget.bill;
    newBill.category = widget.bill.category;
    newBill.billId = widget.bill.billId;
    _daysBeforeController.text = widget.bill.daysBefore.toString();
    _categoryController.text = newBill.category.name.toString();
    _newAmountController.text = newBill.amount.toString();
    _noteController.text = newBill.note.toString();
    _nameController.text = newBill.name;
  ...
  void _validateCategory() {
    setState(() {
      _isCategorySelected = newBill.category != Category.empty;
    });
  }
                  ...
                  Stack(
                    ...
                    children: [
                      TextFormField(
                        ...
                        decoration: InputDecoration(
                            filled: true,
                            fillColor: newBill.category == Category.empty
                                ? Colors.white
                                : Color(newBill.category.color),
                            prefixIcon: newBill.category == Category.empty
                                ? const Icon(
                                ...
                                : Image.asset(
                                    'assets/${newBill.category.icon}.png',
                                    scale: 2,
                                  ),
                            hintText: 'Category',
                      ...
                      _isCategorySelected
                          ? IconButton(
                              onPressed: () {
                                setState(() {
                                  newBill.category = Category.empty;
                              ...
                              onPressed: _newFormKey.currentState?.validate() ==
                                  ...
                                  ? () {
                                      ...
                                      if (user != null) {
                                        ...
                                        setState(() {
                                          print(newBill.category.name);
                                          newBill.amount = double.parse(
                                              _newAmountController.text);

                                          newBill.name = _nameController.text;
                                          newBill.user = currentUser;
                                          newBill.frequency =
                                              _selectedFrequency;
                                          allFilled = true;
                                          newBill.daysBefore = int.parse(
                                              _daysBeforeController.text);
                                          newBill.note = _noteController.text;
                                        });
                                        newBill.sceduleNotifications();
                                        context
                                            .read<BillsBloc>()
                                            .add(EditBill(newBill));
                                        Navigator.pop(context);
                                      }
                                    }
                                  : () {
